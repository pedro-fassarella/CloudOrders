...
internal sealed partial class ServiceBusNotificationWorker(
    ServiceBusClient serviceBusClient,
    ServiceBusMessagingOptions options,
    NotificationMessageProcessor notificationMessageProcessor,
    ILogger<ServiceBusNotificationWorker> logger) : BackgroundService
{
    ...
    {
        return notificationMessageProcessor.ProcessAsync(
    ...
    [LoggerMessage(
        ...
        Message = "Notification processor started for topic {TopicName} and subscription {SubscriptionName}.")]
    private partial void LogProcessorStarted(string topicName, string subscriptionName);

    [LoggerMessage(
        Level = LogLevel.Error,
        Message = "Notification processor error from {ErrorSource} for {EntityPath}.")]
