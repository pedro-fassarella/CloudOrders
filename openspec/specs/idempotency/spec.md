...
If business processing or processed-state persistence fails before commit, the worker MUST NOT complete the Service Bus message and a failed business operation MUST NOT be committed as `Completed`. Contract failures MUST create no inbox state. Typed unsupported-status failures MUST roll back the inbox transaction before manual dead-lettering.

#### Scenario: Transient business processing fails
...
#### Scenario: Contract failure is dead-lettered

- **WHEN** a message fails contract validation or deserialization before processed-message processing begins
- **THEN** no completed inbox state MUST be persisted
- **AND** the message MUST be immediately dead-lettered with reason `MessageContractViolation`.
...
- **AND** the message MUST be manually dead-lettered with reason `UnsupportedBusinessStatus` without a completed inbox state.
